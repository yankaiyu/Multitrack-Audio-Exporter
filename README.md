...
- Create a ZIP share package containing only the tracks selected for the current export.
...
- Choose conservative (1), balanced (2, default), or fast (4) concurrent tracks for a single export job.
- Show approximate stage progress for cleaning, encoding, peak validation, and ZIP creation.
- Safely serialize simultaneous exports targeting the same source folder.
...
Only one local service runs at a time. Opening `start.command` again reuses an existing app service and opens it in the browser instead of causing a port conflict. When the final app page is closed, the local server automatically exits after about one minute; an active export is allowed to finish first.
...
- The Homebrew FFmpeg formula is installed in Homebrew's own prefix and does not modify other FFmpeg installations.
...
The red **Uninstall app-managed dependencies** button removes only FFmpeg that this app itself installed. Its record includes the exact Homebrew executable, Homebrew prefix, and FFmpeg formula prefix; all three must match before uninstalling. Homebrew's indirect dependencies are deliberately retained to avoid affecting other tools.
...
- 可选择保守（1 条）、平衡（2 条，默认）或快速（4 条）作为同一导出任务内的并发轨道数。
- 显示清洗、编码、峰值验证与创建 ZIP 的大致阶段进度。
- 同一源文件夹的多个转换任务会安全串行，避免输出竞争。
...
一次只能运行一个本地服务。再次双击 `start.command` 时，工具会复用已运行的服务并打开浏览器，不会产生端口冲突。关闭最后一个本工具网页后，本地服务会在约一分钟后自动退出；若正在转换，则会先等待转换完成。
...
- Homebrew 安装的 FFmpeg 位于 Homebrew 自身前缀内，不会改写其他位置的 FFmpeg 安装。
...
红色的 **卸载本工具安装的依赖** 按钮只会删除本工具安装的 FFmpeg。记录会保存精确的 Homebrew 可执行文件、Homebrew 前缀与 FFmpeg 公式前缀；卸载前必须全部匹配。Homebrew 的间接依赖会刻意保留，避免影响其他工具。
